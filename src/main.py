...
def main(args):
    if args.train:
        ...
        generated_java_code = generator.generate_java(args.prompt)
        print(generated_java_code)
    elif args.prune:
        _model_loader = CausalLmLoader(args.saved_model_path)
        ...
        quantizer = Quantizer(args.saved_model_path)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Fine-tune LLM for signature verification code generation")
    ...
    parser.add_argument("--prompt", type=str, help="The prompt to request code generation")
